...
router.put("/ocuparDesocupar/:id", async (req, res) => {
  ...
  await mesas
    ...
    .then((data) =>
      res.status(200).json({ mensaje: "Estado de la mesa actualizado" }),
    )
    .catch((error) => res.json({ message: error }));
});
...
router.put("/actualizar/:id", async (req, res) => {
  ...
  await mesas
    .updateOne(
      { _id: id },
      { $set: { numeroMesa, descrpcion, numeroPersonas } },
    )
    .then((data) =>
      res.status(200).json({ mensaje: "Datos de la mesa actualizados" }),
    )
    .catch((error) => res.json({ message: error }));
});
